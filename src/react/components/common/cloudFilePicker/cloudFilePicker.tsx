...
> {
    ...
    public render() {
        ...
        return (
            <Modal isOpen={this.state.isOpen} centered={true}>
                    ...
                    <>
                        <div className={"shared-string-input-container"}>
                            ...
                            <InputGroup className="input-uri">
                                <Input
                                    placeholder={
                                        strings.homePage.openCloudProject
                                            .pasteSharedUri
                                    }
                                    id="sharedURI"
                                    type="text"
                ...
                {!this.state.selectedConnection && !this.state.pastedUri && (
    ...
    private async ok() {
        if (this.state.selectedConnection && this.state.selectedFile) {
            ...
            const content = await storageProvider.readText(
                this.state.selectedFile
            );
            ...
            if (sharedConnection) {
                const storageProvider =
                    StorageProviderFactory.createFromConnection(
                        sharedConnection.connection
                    );
                const content = await storageProvider.readText(
                    sharedConnection.projectName + ".fott"
                );
    ...
    private getSharedProjectConnectionInfo() {
        if (this.getSharedUriParams(this.state.pastedUri)) {
            const { token, sasFolder, projectName } = this.getSharedUriParams(
                this.state.pastedUri
            );
    ...
    private getSharedConnection(connections: IConnection[], sasFolder: string) {
        const connection: IConnection = connections?.find(
            ({ providerOptions }) => providerOptions["sas"]?.includes(sasFolder)
        );
        if (connection) {
            return connection;
        }
    ...
    private isCloudConnection(connection: IConnection): boolean {
        try {
            const storageProvider =
                StorageProviderFactory.createFromConnection(connection);
    ...
    private connectionList() {
        ...
        return this.getCondensedList("Cloud Connections", connections, (args) =>
            this.onClickConnection(args)
        );
    }
    ...
    private async fileList(connection: IConnection) {
        ...
        return this.getCondensedList(
            `${this.props.fileExtension || "All"} Files in "${
                connection.name
            }"`,
            fileItems,
            this.onClickFile
