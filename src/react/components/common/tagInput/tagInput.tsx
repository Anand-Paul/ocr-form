...
export class TagInput extends React.Component<ITagInputProps, ITagInputState> {
    ...
    private tagItemRefs: Map<string, TagInputItem> = new Map<
        string,
        TagInputItem
    >();
    ...
    public componentDidUpdate(prevProps: ITagInputProps) {
        if (prevProps.tags !== this.props.tags) {
            ...
            if (selectedTag) {
                selectedTag = this.props.tags.find((tag) =>
                    isNameEqual(tag.name, selectedTag.name)
                );
            }
        ...
        if (this.props.tagInputMode === TagInputMode.ConfigureTable) {
            return (
                <div className="tag-input">
                    <div className="tag-input-header p-2">
                        <span className="tag-input-title">
                            {strings.tags.title}
                        </span>
                    </div>
                    <div className="tag-input-body-container">
                        <div className="tag-input-body">
                            <TableTagConfig
                                ...
                                reconfigureTableConfirm={
                                    this.props.reconfigureTableConfirm
                                }
                                selectedTableBody={
                                    this.props.selectedTableTagBody
                                }
                            />
                        </div>
                    </div>
            ...
            return (
                <div className="tag-input">
                    ...
                    <TableTagLabeling
                        ...
                        setTagInputMode={this.props.setTagInputMode}
                        selectedTag={
                            this.props.selectedTableTagToLabel as ITableTag
                        }
                        handleTableCellClick={this.props.handleTableCellClick}
                        handleTableCellMouseEnter={
                            this.props.handleTableCellMouseEnter
                        }
            ...
            return (
                <div className="tag-input">
                    <div ref={this.headerRef} className="tag-input-header p-2">
                        ...
                        <TagInputToolbar
                            ...
                            onOnlyCurrentPageTags={() =>
                                this.setState({
                                    onlyCurrentPageTags:
                                        !this.state.onlyCurrentPageTags,
                                })
                            }
                    ...
                    <Confirm
                        title={
                            strings.tags.warnings.replaceAllExitingLabelsTitle
                        }
    ...
    private onLockTag = (tag: ITag) => {
        ...
        if (lockedTags.find((str) => isNameEqual(tag.name, str))) {
            lockedTags = lockedTags.filter(
                (str) => !isNameEqual(tag.name, str)
            );
        } else {
    ...
    private onTagRename = (
        ...
        try {
            const tagsWithoutOldTag = this.state.tags.filter(
                (elem) => !isNameEqual(elem.name, tag.name)
            );
    ...
    private getColorPickerPortal = () => {
        ...
        const showColorPicker =
            this.state.tagOperation === TagOperationMode.ColorPicker;
        return (
            <AlignPortal
                align={{ points: ["tr", "tl"] }}
    ...
    private setTagLabels = (key: string): any[] => {
        const labels = this.props.labels.filter((label) => {
            if (this.props.encoded) {
                return (
                    label.label.replace(/~1/g, "/").replace(/~0/g, "~") === key
                );
            } else {
                return label.label === key;
            }
        });
        const tableLables = this.props.tableLabels.filter(
            (label) => label.tableKey === key
        );
        return [...labels, ...tableLables];
    };

    private createTagItemProps = (): ITagInputItemProps[] => {
        const { tags, selectedTag, tagOperation, onlyCurrentPageTags } =
            this.state;
        ...
        if (onlyCurrentPageTags) {
            ...
            const tableLabels = this.props.tableLabels
                .filter(
                    (item) =>
                        item.labels[0]?.value[0]?.page === this.props.pageNumber
                )
            ...
            if (labeledTags.length) {
                return tags
                    ...
                    .map<ITagInputItemProps>((tag) => {
                        return {
                            tag,
                            index: tags.findIndex((t) =>
                                isNameEqual(t.name, tag.name)
                            ),
                            isLocked:
                                this.props.lockedTags &&
                                this.props.lockedTags.findIndex((str) =>
                                    isNameEqual(tag.name, str)
                                ) > -1,
                            isRenaming:
                                selectedTag &&
                            ...
                            isSelected:
                                selectedTag &&
                                isNameEqual(selectedTag.name, tag.name),
                            appliedToSelectedRegions: selectedRegionTagSet.has(
                                tag.name
                            ),
            ...
            (tag) =>
                ({
                    ...
                    isRenaming:
                        ...
                        tagOperation === TagOperationMode.Rename,
                    isSelected:
                        selectedTag && isNameEqual(selectedTag.name, tag.name),
                    appliedToSelectedRegions: selectedRegionTagSet.has(
                        tag.name
                    ),
                    onClick: this.onTagItemClick,
    ...
    private onTagItemClick = (tag: ITag, props: ITagClickProps) => {
        if (props.ctrlKey && this.props.onCtrlTagClick) {
            ...
            const { selectedTag, tagOperation } = this.state;
            const showColorPicker =
                tagOperation !== TagOperationMode.ColorPicker;
            ...
            const selected =
                selectedTag && isNameEqual(selectedTag.name, tag.name);
            const tagOperation = selected
                ? oldTagOperation
                : TagOperationMode.None;
            let deselect =
                selected && oldTagOperation === TagOperationMode.None;
            ...
            if (selectedRegions && selectedRegions.length && onTagClick) {
                ...
                const isTagLabelTypeDrawnRegion = this.labelAssignedDrawnRegion(
                    labels,
                    tag.name
                );
    ...
    focusTag(tag: string) {
        ...
        if (tagItemRef) {
            tagItemRef.current.scrollIntoView({
                behavior: "smooth",
                block: "nearest",
            });
    ...
    private creatTagInput = (value: any) => {
        ...
        if (
            newTag.name.length &&
            ![...this.state.tags, newTag].containsDuplicates((t) => t.name)
        ) {
            this.addTag(newTag);
    ...
    private getContextualMenuItems = (): IContextualMenuItem[] => {
        ...
        const menuItems: IContextualMenuItem[] = [
            ...
            },
            {
                ...
                text: strings.tags.regionTableTags.tableLabeling.buttons
                    .reconfigureTable,
                onClick: () =>
                    this.props.setTagInputMode(
                        TagInputMode.ConfigureTable,
                        tag as ITableTag
                    ),
            },
            {
    ...
    private getTypeSubMenuItems = (): IContextualMenuItem[] => {
        ...
        if (tag.type === FieldType.Object || tag.type === FieldType.Array) {
            ...
            types = types.filter(
                (i) => i !== FieldType.Array && i !== FieldType.Object
            );
        }
    ...
    public triggerRenameTagBlur() {
        if (
            ...
            this.tagItemRefs.get(this.state.selectedTag.name)
        ) {
