...
export class ImageMap extends React.Component<IImageMapProps> {
    ...
    private readonly TABLE_ICON_BORDER_VECTOR_LAYER_NAME =
        "tableIconBorderVectorLayer";
    ...
    private readonly DRAWN_REGION_LABEL_VECTOR_LAYER_NAME =
        "drawnRegionLabelledVectorLayer";
    ...
    private imageLayerFilter = {
        layerFilter: (layer: Layer) =>
            layer.get("name") === this.IMAGE_LAYER_NAME,
    };

    private textVectorLayerFilter = {
        layerFilter: (layer: Layer) =>
            layer.get("name") === this.TEXT_VECTOR_LAYER_NAME,
    };

    private checkboxLayerFilter = {
        layerFilter: (layer: Layer) =>
            layer.get("name") === this.CHECKBOX_VECTOR_LAYER_NAME,
    };

    private tableIconBorderVectorLayerFilter = {
        layerFilter: (layer: Layer) =>
            layer.get("name") === this.TABLE_ICON_BORDER_VECTOR_LAYER_NAME,
    };

    private labelVectorLayerFilter = {
        layerFilter: (layer: Layer) =>
            layer.get("name") === this.LABEL_VECTOR_LAYER_NAME,
    };

    private drawnLabelVectorLayerFilter = {
        layerFilter: (layer: Layer) =>
            layer.get("name") === this.DRAWN_REGION_LABEL_VECTOR_LAYER_NAME,
    };

    private drawnRegionVectorLayerFilter = {
        layerFilter: (layer: Layer) =>
            layer.get("name") === this.DRAWN_REGION_VECTOR_LAYER_NAME,
    };
    ...
    public componentDidUpdate(prevProps: IImageMapProps) {
        if (this.props.initEditorMap || this.props.initLayoutMap) {
            ...
            if (
                !this.props.isPointerOnImage &&
                prevProps.isPointerOnImage &&
                this.props.isVertexDragging
            ) {
                this.cancelModify();
            }
        }
        ...
        if (
            prevProps.imageUri !== this.props.imageUri ||
            prevProps.imageAngle !== this.props.imageAngle
        ) {
            this.imageExtent = [
                0,
                0,
    ...
    public toggleTableFeatureVisibility = (visible: boolean = false) => {
        this.tableBorderVectorLayer.setVisible(
            visible || !this.tableBorderVectorLayer.getVisible()
        );
        this.tableIconVectorLayer.setVisible(
            visible || !this.tableIconVectorLayer.getVisible()
        );
        this.tableIconBorderVectorLayer.setVisible(
            visible || !this.tableIconBorderVectorLayer.getVisible()
        );
    };

    public toggleLabelFeatureVisibility = (visible: boolean = false) => {
        this.labelVectorLayer.setVisible(
            visible || !this.labelVectorLayer.getVisible()
        );
        let drawLabelVectorLayerVisibility =
            this.drawnLabelVectorLayer.getVisible();
        this.drawnLabelVectorLayer.setVisible(
            visible || !drawLabelVectorLayerVisibility
        );
        drawLabelVectorLayerVisibility =
            this.drawnLabelVectorLayer.getVisible();
    ...
    public toggleDrawnRegionsFeatureVisibility = (visible: boolean = false) => {
        let drawRegionVectorLayerVisibility =
            this.drawRegionVectorLayer.getVisible();
        this.drawRegionVectorLayer.setVisible(
            visible || !drawRegionVectorLayerVisibility
        );
    ...
    private pushToDrawnFeatures = (
        feature,
        drawnFeatures: Collection = this.drawnFeatures
    ) => {
        const itemAlreadyExists =
            drawnFeatures.getArray().indexOf(feature) !== -1;
        if (!itemAlreadyExists) {
            drawnFeatures.push(feature);
        }
    };

    private removeFromDrawnFeatures = (
        feature,
        drawnFeatures: Collection = this.drawnFeatures
    ) => {
        const itemAlreadyExists =
    ...
    public toggleCheckboxFeatureVisibility = (visible: boolean = false) => {
        this.checkboxVectorLayer.setVisible(
            visible || !this.checkboxVectorLayer.getVisible()
        );
    };
    ...
    public toggleTextFeatureVisibility = (visible: boolean = false) => {
        this.textVectorLayer.setVisible(
            visible || !this.textVectorLayer.getVisible()
        );
    };
    ...
    public addInteraction = (interaction: Interaction) => {
        if (
            ...
            this.map.getInteractions().array_.find((existingInteraction) => {
                return (
                    interaction.constructor === existingInteraction.constructor
                );
            })
        ) {
    ...
    public getTableBorderFeatureByID = (featureID) => {
        return this.tableBorderVectorLayer
            .getSource()
    ...
    public getTableIconBorderFeatureByID = (featureID) => {
        return this.tableIconBorderVectorLayer
            .getSource()
            .getFeatureById(featureID);
    };
    ...
    public removeInteraction = (interaction: Interaction) => {
        const existingInteraction = this.map
            .getInteractions()
            .array_.find((existingInteraction) => {
                return (
                    interaction.constructor === existingInteraction.constructor
                );
            });
    ...
    private setImage = (imageUri: string, imageExtent: number[]) => {
        ...
        this.imageLayer.setSource(
            this.createImageSource(imageUri, projection, imageExtent)
        );
    ...
    private getMinimumZoom = () => {
        ...
        const containerAspectRatio = this.mapElement
            ? this.mapElement.clientHeight / this.mapElement.clientWidth
            : 1;
    ...
    private handleDoubleClick = (event: MapBrowserEvent) => {
        ...
        if (filter && this.props.handleFeatureDoubleClick) {
            this.map.forEachFeatureAtPixel(
                eventPixel,
                (feature) => {
                    this.props.handleFeatureDoubleClick(
                        feature,
                        true,
                        filter.category
                    );
    ...
    private handlePointerMoveOnTableIcon = (event: MapBrowserEvent) => {
        if (this.props.handleTableToolTipChange) {
            const eventPixel = this.map.getEventPixel(event.originalEvent);
            const isPointerOnTableIconFeature = this.map.hasFeatureAtPixel(
                eventPixel,
            ...
            if (isPointerOnTableIconFeature) {
                ...
                if (features.length > 0) {
                    ...
                    if (
                        feature &&
                        this.props.hoveringFeature !== feature.get("id")
                    ) {
    ...
    public cancelModify = () => {
        Object.entries(this.modifyStartFeatureCoordinates).forEach(
            (featureCoordinate) => {
                let feature = this.getDrawnRegionFeatureByID(
                    featureCoordinate[0]
                );
                if (!feature) {
                    feature = this.getDrawnLabelFeatureByID(
                        featureCoordinate[0]
                    );
                }
                if (
                    feature.getGeometry().flatCoordinates.join(",") !==
                    featureCoordinate[1]
                ) {
                    const oldFlattenedCoordinates = (
                        featureCoordinate[1] as string
                    )
                        .split(",")
                        .map(parseFloat);
                    ...
                    for (
                        let i = 0;
                        i < oldFlattenedCoordinates.length;
                        i += 2
                    ) {
                        oldCoordinates.push([
    ...
    private initializeModify = () => {
        ...
        this.modify.on("modifystart", (modifyEvent) => {
            ...
            features.forEach((feature) => {
                ...
                this.modifyStartFeatureCoordinates[feature.getId()] =
                ...
                this.labelVectorLayer
                    ...
                    .forEachFeatureInExtent(extent, (feature) => {
                        const selectedRegion =
                            this.props.handleFeatureSelectByGroup(feature);
                        if (selectedRegion) {
            ...
            if (this.textVectorLayer.getVisible()) {
                this.textVectorLayer
                    ...
                    .forEachFeatureInExtent(extent, (feature) => {
                        ...
                        if (
                            selectedRegion &&
                            !featureMap.hasOwnProperty(feature.get("id"))
                        ) {
                            regionsToAdd.push(selectedRegion);
                        }
                    });
    ...
    private initializeSnapCheck = () => {
        const snapCheck = new Interaction({
            handleEvent: (evt: MapBrowserEvent) => {
                if (
                    !this.props.isVertexDragging &&
                    this.props.handleIsSnapped
                ) {
                    this.props.handleIsSnapped(
                        this.snap.snapTo(evt.pixel, evt.coordinate, evt.map)
                            .snapped && this.props.isPointerOnImage
                    );
                }
                return true;
            },
    ...
    private getCursor = () => {
        if (this.props.initEditorMap) {
            if (this.props.isVertexDragging) {
                ...
                this.props?.groupSelectMode ||
                this.props?.drawRegionMode
            ) {
                if (this.props.isPointerOnImage) {
                    return "crosshair";
                } else {
    ...
    private initializeTableLayers = () => {
        ...
        this.tableIconBorderVectorLayer = new VectorLayer(
            tableIconBorderOptions
        );
    };
