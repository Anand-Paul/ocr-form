...
export interface ITrainPageProps
    extends RouteComponentProps,
        React.Props<TrainPage> {
...
> {
    ...
    public render() {
        ...
        const trainDisabled: boolean =
            ...
            (this.state.inputtedLabelFolderURL.length === 0 ||
                this.state.inputtedLabelFolderURL ===
                    strings.train.defaultLabelFolderURL);
        ...
        return (
            <div className="train-page skipToMainContent" id="pageTrain">
                <main className="train-page-main">
                    {currTrainRecord && (
                        <div>
                            ...
                            <span>
                                {" "}
                                Model ID: {
                                    currTrainRecord.modelInfo.modelId
                                }{" "}
                            </span>
                        </div>
                    )}
                    {this.state.viewType === "tableView" && (
                        <TrainTable
                            ...
                            accuracies={
                                currTrainRecord && currTrainRecord.accuracies
                            }
                        />
                    )}
                                ...
                                {localFileSystemProvider && (
                                    <div>
                                        ...
                                        <TextField
                                            ...
                                            onFocus={
                                                this
                                                    .removeDefaultInputtedLabelFolderURL
                                            }
                                            onChange={
                                                this.setInputtedLabelFolderURL
                                            }
                                            value={
                                                this.state
                                                    .inputtedLabelFolderURL
                ...
                <Alert
                    ...
                    onClose={() =>
                        this.setState({ showTrainingFailedWarning: false })
                    }
                />
                <PreventLeaving
                    when={this.state.isTraining}
                    message={
                        "A training operation is currently in progress, are you sure you want to leave?"
                    }
                />
                <Confirm
    ...
    private handleViewTypeClick = (
        viewType: "tableView" | "chartView"
    ): void => {
    ...
    private async triggerJsonDownload(): Promise<any> {
        ...
        const modelUrl = this.state.modelUrl.length
            ? this.state.modelUrl
            : currModelUrl;
        const modelJSON = await this.getModelsJson(
            this.props.project,
            modelUrl
        );
