...
> {
    ...
    state: IPrebuiltPredictPageState = {
        ...
        predictionEndpointUrl:
            "/formrecognizer/v2.1/prebuilt/invoice/analyze?includeTextDetails=true",
    ...
    public async componentDidMount() {
        ...
        document.title =
            strings.prebuiltPredict.title + " - " + strings.appName;
    ...
    componentDidUpdate(
        _prevProps: IPrebuiltPredictPageProps,
        prevState: IPrebuiltPredictPageState
    ) {
        if (this.state.file) {
            ...
            if (
                this.getOcrFromAnalyzeResult(this.state.analyzeResult).length >
                    0 &&
            ...
            if (prevState.highlightedField !== this.state.highlightedField) {
                this.setPredictedFieldHighlightStatus(
    ...
    public render() {
        ...
        const onPrebuiltsPath: boolean =
            this.props.match.path.includes("prebuilts");

        return (
            <div
                className={`predict skipToMainContent ${
                    onPrebuiltsPath ? "" : "hidden"
                } `}
                id="pagePredict"
                ...
                <div className="predict-sidebar bg-lighter-1">
                    <div className="condensed-list">
                        <h6 className="condensed-list-header bg-darker-2 p-2 flex-center">
                            ...
                            <span>
                                {interpolate(
                                    strings.prebuiltPredict
                                        .anlayWithPrebuiltModels,
                        ...
                        {!this.state.liveMode && (
                            <div className="p-3" style={{ marginTop: "-2rem" }}>
                                <PredictionFilePicker
                                    ...
                                    onSelectSourceChange={
                                        this.onPredictionSelectSourceChange
                                    }
                                ...
                                >
                                    <div className="formtype-section">
                                        ...
                                        <Dropdown
                                            ...
                                            options={this.prebuiltTypes.map(
                                                (type) => ({
                                                    key: type.name,
                                                    text: type.name,
                                                })
                        ...
                        {Object.keys(predictions).length > 0 && (
                            <PredictResult
                                ...
                                downloadPrefix={
                                    this.state.currentPrebuiltType.name
                                }
                                downloadResultLabel={this.state.fileLabel}
                                onPredictionClick={this.onPredictionClick}
                                onPredictionMouseEnter={
                                    this.onPredictionMouseEnter
                                }
                                onPredictionMouseLeave={
                                    this.onPredictionMouseLeave
                                }
                                onTablePredictionClick={
                                    this.onTablePredictionClick
                                }
                            />
                        )}
                        {Object.keys(predictions).length === 0 &&
                            this.state.predictionLoaded && (
                                <div>
                                    {
                                        strings.prebuiltPredict
                                            .noFieldCanBeExtracted
                                    }
                                </div>
                            )}
                        {this.state.viewRegionalTable && (
                            <TableView
                                handleTableViewClose={
                                    this.onTablePredictionClose
                                }
    ...
    private onPrebuiltTypeChange = (e, option: IDropdownOption) => {
        const currentPrebuiltType = this.prebuiltTypes.find(
            (type) => type.name === option.key
        );
        if (
            currentPrebuiltType &&
            this.state.currentPrebuiltType.name !== currentPrebuiltType.name
        ) {
            this.setState(
                {
                    currentPrebuiltType,
    ...
    private createBoundingBoxVectorFeature = (
        ...
        const isHighlighted =
            ...
            this.state.highlightedField?.toLocaleLowerCase();
    ...
    private drawPredictionResult = (): void => {
        ...
        const isCurrentPage = (result) =>
            result.page === this.state.currentPage;
        const ocrForCurrentPage: any = this.getOcrFromAnalyzeResult(
        ...
        if (ocrForCurrentPage) {
            ...
            const createFeature = (fieldName, field) => {
                if (field && field.type === "array") {
                    field.valueArray?.forEach((row) =>
                        createFeature(fieldName, row)
                    );
                } else {
            ...
            if (isSupportField(displayName)) {
                switch (_.get(fieldProps, "type", "")) {
                    ...
                    case "object": {
                        ...
                        for (const [fieldName, objFieldProps] of Object.entries(
                            valueObject || {}
                        )) {
                            flatFieldProps(
        ...
        for (const feature of features) {
            ...
            if (
                ...
                fieldName.toLocaleLowerCase() ===
                    highlightedField.toLocaleLowerCase()
            ) {
    ...
    private handleUpdateRequestURI = (updateEmptyQuery: boolean = true) => {
        this.setState({
            predictionEndpointUrl: this.getUpdatedRequestURI(
                false,
                updateEmptyQuery
            ),
        });
    ...
    private getUpdatedRequestURI = (
        fromTextArea: boolean = false,
        updateEmptyQuery: boolean = true
    ) => {
        ...
        const newQueryString = this.getUpdatedQueryString(
            queryString,
            updateEmptyQuery
        );
    ...
    private getUpdatedPath(path: string, fromTextArea: boolean): string {
        const pathTemplate =
            "/formrecognizer/:prebuiltServiceVersion/prebuilt/:prebuiltType/analyze";
        ...
        if (fromTextArea) {
            ...
            if (
                ...
                const ret = this.prebuiltTypes.filter(
                    (item) => item.servicePath === prebuiltType
                );
                if (ret.length === 1) {
            ...
            pathParams["prebuiltType"] =
                this.state.currentPrebuiltType.servicePath;
        }
    ...
    private getUpdatedQueryString(
        queryString: string,
        updateEmptyQuery: boolean
    ): string {
        let newQueryString = "";
        if (queryString) {
            const parameterArray = queryString.includes("&")
                ? queryString.split("&")
                : [queryString];
            let connector = "";
            ...
            newQueryString += this.state.currentPrebuiltType.useLocale
                ? `${connector}locale=${this.state.currentLocale}`
                : "";
        }
    ...
    private onTablePredictionClick = (
        predictedItem: ITableResultItem,
        tagColor: string
    ) => {
        const makeTable = (clickedFieldName) => {
            ...
            const collectHeaders = (valueArray) => {
                ...
                valueArray.map((item) =>
                    Object.keys(_.get(item, "valueObject", [])).map((column) =>
                        headers.add(column)
                    )
                );
                return headers;
            };
            const columnNames = reOrderColumnHeaders(
                collectHeaders(valueArray)
            );
            const columnHeaders = (function makeColumnHeaders() {
                const indexColumn = new Cell(0, 0, "");
                const contentColumns = columnNames.map(
                    (columnName, columnIndex) =>
                        new Cell(0, columnIndex + 1, columnName, null, true)
                );
            ...
            const flattenCells = matrix.reduce(
                (cells, row) => (cells = [...cells, ...row]),
                []
            );
            return {
                cells: flattenCells,
        ...
        const predictions = this.getPredictionsFromAnalyzeResult(
            this.state.analyzeResult
        );
