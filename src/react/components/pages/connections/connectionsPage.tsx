...
> {
    ...
    public render() {
        return (
            <div
                className="app-connections-page skipToMainContent"
                id="pageConnections"
            >
                <div className="app-connections-page-list bg-lighter-1">
                    <CondensedList
                        ...
                        onDelete={(connection) =>
                            this.confirmDelete.current.open(connection)
                        }
    ...
    private async loadConnection(connectionId: string) {
        let connection = this.props.connections.find(
    ...
    private onConnectionDelete = async (connection: IConnection) => {
        ...
        toast.info(
            interpolate(strings.connections.messages.deleteSuccess, {
                connection,
            })
        );
        ...
        try {
            if (connection.providerType === "azureBlobStorage") {
                connection.providerOptions["sas"] =
                    connection.providerOptions["sas"].trim();
            }
            ...
            if (
                ...
                (connection.id &&
                    this.props.connections.find(
                        (a) =>
                            a.name === connection.name && a.id !== connection.id
                    ))
            ) {
                toast.error(
                    interpolate(
                        strings.connections.messages.doNotAllowDuplicateNames,
                        { connection }
                    ),
                    { autoClose: 10000 }
                );
