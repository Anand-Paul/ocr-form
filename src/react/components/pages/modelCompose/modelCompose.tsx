...
export interface IModelComposePageProps
    extends RouteComponentProps,
        React.Props<ModelComposePage> {
...
export default class ModelComposePage extends React.Component<
    IModelComposePageProps,
    IModelComposePageState
> {
    ...
    private composeModalRef: React.RefObject<ComposeModelView> =
        React.createRef();
    ...
    constructor(props) {
        ...
        const columns: IColumn[] = [
            ...
            },
            {
                ...
                onRender: (model: IModel) => (
                    <span>
                        {new Date(model.lastUpdatedDateTime).toLocaleString()}
                    </span>
                ),
            },
        ];
    ...
    onRenderRow = (
        ...
        return (
            ...
            defaultRender({
                ...props,
                className: `${modelNotReady ? "model-not-ready" : ""}`,
            })
        );
    };
        ...
        return (
            <>
                            ...
                            >
                                ...
                                >
                                    {this.state.isComposing ? (
                                        <Spinner
                                            label={
                                                strings.modelCompose.composing
                                            }
                ...
                <PreventLeaving
                    ...
                    message={
                        "A composing operation is currently in progress, are you sure you want to leave?"
                    }
                />
            </>
        );
    ...
    private onItemInvoked = async (model: IModel, index: number, ev: Event) => {
        ...
        if (model.attributes.isComposed) {
            ...
            for (const i of Object.keys(inclModels)) {
                ...
                try {
                    _model = await this.getModelByURl(
                        interpolate(constants.apiModelsPath, { apiVersion }) +
                            "/" +
                            inclModels[i].modelId
                    );
                    modelInfo = {
                        id: _model.modelId,
    ...
    private returnReadyModels = (modelList) =>
        modelList.filter(
            (model: IModel) => model.status === constants.statusCodeReady
        );
    ...
    private getModelList = async () => {
        try {
            ...
            models = models.filter(
                (model) => recentModelIds.indexOf(model.modelId) === -1
            );
    ...
    private getRecentModels = async (): Promise<IModel[]> => {
        ...
        const apiVersion = getAPIVersion(this.props.project?.apiVersion);
        const recentModelRequest = await allSettled(
            this.props.project.recentModelRecords.map(async (model) => {
                return this.getModelByURl(
                    interpolate(constants.apiModelsPath, { apiVersion }) +
                        "/" +
        ...
        try {
            if (this.allModels.length <= 5000) {
                if (this.state.nextLink.length !== 0) {
                    ...
                    const nextPage = await this.getModelsFromNextLink(
                        this.state.nextLink
                    );
                    const currentList = this.state.modelList;
                    const recentModelIds = this.getRecentModelIds(
                        this.state.recentModelsList
                    );

                    let nextPageList = nextPage.nextList;
                    nextPageList = nextPageList.filter(
                        (model) => recentModelIds.indexOf(model.modelId) === -1
                    );
                    const appendedList = currentList.concat(nextPageList);
                    const currerntlySortedColumn: IColumn =
                        this.state.columns.find((column) => column.isSorted);
                    const appendedAndSortedList = this.copyAndSort(
                        appendedList,
                        currerntlySortedColumn?.fieldName,
                        currerntlySortedColumn?.isSortedDescending
                    );
    ...
    private async getResponse(nextLink?: string) {
        ...
        const baseURL =
            nextLink === undefined
                ...
                : url.resolve(this.props.project.apiUriBase, nextLink);
        const config = {
            headers: {
    ...
    private handleColumnClick = (
        ...
        const newList = this.copyAndSort(
            modelList,
    ...
    private copyAndSort(
        ...
        const key = columnKey;
        if (
            key === strings.modelCompose.column.created.fieldName ||
            key === strings.modelCompose.column.lastUpdated.fieldName
        ) {
            return modelList.slice(0).sort((a, b): number => {
            ...
            return modelList.slice(0).sort((a, b) => {
        ...
        this.setState({
            modelList: text
                ? this.allModels.filter(
                      ({ modelName, modelId }) =>
                          modelId.indexOf(text.toLowerCase()) > -1 ||
                          (modelName !== undefined
                              ? modelName
                                    .toLowerCase()
    ...
    private onComposeButtonClick = () => {
        this.composeModalRef.current.open(
            this.selectedItems,
            this.cannotBeIncludedItems,
            true
        );
    };
    ...
    private addToRecentModels = async (modelToAdd: IRecentModel) => {
        ...
        if (
            recentModelRecords.find(
                (recentModel) =>
                    recentModel.modelInfo.modelId ===
                    modelToAdd.modelInfo.modelId
            )
        ) {
            if (modelToAdd.modelInfo.modelName) {
                toast.success(
                    interpolate(
                        ...
                        { modelID: modelToAdd.modelInfo.modelName }
                    )
                );
            } else {
                toast.success(
                    interpolate(
                        strings.modelCompose.modelView
                            .recentModelsAlreadyContainsModel,
                        { modelID: modelToAdd.modelInfo.modelId }
                    )
                );
            }
        ...
        if (modelToAdd.modelInfo.modelName) {
            toast.success(
                interpolate(
                    strings.modelCompose.modelView.addModelToRecentModels,
                    { modelID: modelToAdd.modelInfo.modelName }
                )
            );
        } else {
    ...
    private passSelectedItems = (Items: any[]) => {
        this.cannotBeIncludedItems = Items.filter(
            (item: IModel) => item.status !== constants.statusCodeReady
        );
    ...
    private poll = (func, timeout, interval): Promise<any> => {
        ...
        const checkSucceeded = (resolve, reject) => {
            ...
            ajax.then((response) => {
                if (
                    response.data.modelInfo.status.toLowerCase() ===
                    constants.statusCodeReady
                ) {
                    resolve(response.data);
                } else if (
                    response.data.modelInfo.status.toLowerCase() ===
                    constants.statusCodeInvalid
                ) {
                    toast.error(strings.modelCompose.errors.failedCompose, {
                        autoClose: false,
                    });
    ...
    private async waitUntilModelIsReady(
    ...
    private handleModelsCompose = async (selections: any[], name: string) => {
        setTimeout(async () => {
            try {
                ...
                const composedModel = await this.waitUntilModelIsReady(
                    composeRes["headers"]["location"]
                );
                ...
                if (error.errorCode === ErrorCode.ModelNotFound) {
                    this.setState({
                        isError: true,
                        errorMessage:
                            strings.modelCompose.limitQuantityComposedModel,
                    });
                } else {
    ...
    private async post(link, payload): Promise<any> {
        const baseURL = url.resolve(this.props.project.apiUriBase, link);
