...
import {
    ...
    ILoadFileResult,
    LoadFileHelper,
} from "../prebuiltPredict/LoadFileHelper";
import {
    ITableHelper,
    ITableState,
    TableHelper,
} from "../prebuiltPredict/tableHelper";
...
> {
    ...
    private uploadToTrainingSetView: React.RefObject<UploadToTrainingSetView> =
        React.createRef();
    private duplicateAssetNameConfirm: React.RefObject<Confirm> =
        React.createRef();
    ...
    public async componentDidUpdate(
        _prevProps: IPredictPageProps,
        prevState: IPredictPageState
    ) {
        ...
        if (this.state.file) {
            ...
            if (
                prevState.highlightedTableCellColumnKey !==
                    this.state.highlightedTableCellColumnKey ||
                prevState.highlightedTableCellRowKey !==
    ...
    public render() {
        ...
        const modelInfo: IAnalyzeModelInfo = this.getAnalyzeModelInfo(
            this.state.analyzeResult
        );

        const onPredictionPath: boolean =
            this.props.match.path.includes("predict");
            ...
            >
                ...
                >
                    <div className="condensed-list">
                            ...
                            <>
                                {!mostRecentModel ? (
                                    <div className="bg-darker-2 pl-3 pr-3 flex-center ">
                                        <div
                                            className="alert alert-warning warning no-models-warning"
                                            role="alert"
                                        >
                                    ...
                                    <>
                                        ...
                                        >
                                            ...
                                            {this.props.project.apiVersion ===
                                                constants.prebuiltServiceVersion &&
                                        ...
                                        >
                                            ...
                                            <UploadToTrainingSetView
                                                ...
                                                ref={
                                                    this.uploadToTrainingSetView
                                                }
                                                onConfirm={
                                                    this.onAddAssetToProject
                                                }
                                            />
                                            {Object.keys(predictions).length ===
                                                0 &&
                                                this.state.predictRun && (
                                                    <div>
                                                        No field can be
                                                        extracted.
                                                    </div>
                        ...
                        {tagViewMode ===
                            AnalyzedTagsMode.LoadingRecentModel && (
                            <Spinner
                        ...
                        {this.state.viewRegionalTable && (
                            <div className="m-2">
                                <h4 className="ml-1 mb-4">
                                    View analyzed Table
                                </h4>
                                <RegionalTable
                                    regionalTableToView={
                                        this.state.regionalTableToView
                                    }
        ...
        axios
            ...
            .then((response) => {
                ...
                const downloadFileName =
                    fileBaseName + "-" + modelID.substring(0, 4) + ".py";
        ...
        try {
            ...
            if (err.response?.status === 404) {
                throw new AppError(
                    ...
                    interpolate(strings.errors.modelNotFound.message, {
                        modelID,
                    })
                );
            } else {
    ...
    private drawPredictionResult = (): void => {
        ...
        const fields = this.getPredictionsFromAnalyzeResult(
            this.state.analyzeResult
        );
            ...
            if (field?.type === "object" && field.valueObject) {
                Object.keys(field?.valueObject).forEach((rowName, rowIndex) => {
                    if (field?.valueObject?.[rowName]) {
                        Object.keys(
                            field?.valueObject?.[rowName]?.valueObject
                        ).forEach((columnName, colIndex) => {
                            const tableCell =
                                field?.valueObject?.[rowName]?.valueObject?.[
                                    columnName
                                ];
                            if (tableCell?.page === this.state.currentPage) {
                                ...
                                const boundingbox = _.get(
                                    tableCell,
                                    "boundingBox",
                                    []
                                );
                                const feature =
                                    this.createBoundingBoxVectorFeatureForTableCell(
                                        text,
                                        boundingbox,
                                        imageExtent,
                                        ocrExtent,
                ...
                field?.valueArray.forEach((row, rowIndex) => {
                    Object.keys(row?.valueObject).forEach(
                        (columnName, colIndex) => {
                            const tableCell =
                                field?.valueArray?.[rowIndex]?.valueObject?.[
                                    columnName
                                ];
                            if (tableCell?.page === this.state.currentPage) {
                                const text = fieldName;
    ...
    private getAnalyzeModelInfo(analyzeResult) {
        const { modelId, docType, docTypeConfidence } = _.get(
            analyzeResult,
            "documentResults[0]",
            {}
        );
    ...
    private onAddAssetToProjectClick = async () => {
        if (this.state.file) {
            ...
            }/${decodeURIComponent(this.state.file.name)}`;
            const asset = Object.values(this.props.project.assets).find(
    ...
    private onAddAssetToProject = async () => {
        if (this.state.file) {
            ...
            const fileName = decodeURIComponent(this.state.file.name)
                .split("/")
                .pop();
            await this.props.actions.addAssetToProject(
                this.props.project,
                fileName,
                fileData,
    ...
    private setPredictedFieldHighlightStatus = (highlightedField: string) => {
        ...
        for (const feature of features) {
            if (
                feature.get("fieldName").toLocaleLowerCase() ===
    ...
    private handleRecentModelsViewClose = () => {
        ...
        if (selectedIndex !== this.state.selectedRecentModelIndex) {
            this.selectionHandler.setIndexSelected(
                this.state.selectedRecentModelIndex,
                true,
                true
            );
    ...
    private async getRecentModelFromPredictModelId(): Promise<any> {
        ...
        try {
            response = await axios
                .get(endpointURL, {
                    headers: {
                        [constants.apiKeyHeader]: this.props.project
                            .apiKey as string,
                    },
                })
                .catch((err) => {
            ...
            this.setState({
                ...
                alertTitle: "Failed to get recent model",
                alertMessage:
                    "Check network and service URI in project settings",
            });
        }
        if (!response) {
            return;
    ...
    private async updateRecentModelsViewer(project) {
        ...
        project.recentModelRecords.forEach((model: IRecentModel, index) => {
            ...
            recentModelRecordsWithKey[index] = Object.assign(
                { key: index },
                model
            );
        });
