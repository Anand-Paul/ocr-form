...
export default class ServiceHelper {
        ...
        if (err && err.response) {
            if (err.response.status === 401) {
                ...
                if (
                    ...
                    errorMessage =
                        strings.errors.modelCountLimitExceeded.message;
            ...
            toast.warn(
                interpolate(strings.errors.endpointConnectionError.message, {
    ...
    public static postWithAutoRetry = <T>(
        ...
        ServiceHelper.sendRequestWithAutoRetry(() =>
            axios.post(url, data, ServiceHelper.applyApiKey(config, apiKey))
        );
    ...
    public static getWithAutoRetry = <T = any>(
        ...
        ServiceHelper.sendRequestWithAutoRetry(() =>
            axios.get(url, ServiceHelper.applyApiKey(config, apiKey))
        );
    ...
    private static sendRequestWithAutoRetry = async <T>(
        ...
        while (true) {
            try {
                ...
                await delay(
                    constants.initialRetryInterval *
