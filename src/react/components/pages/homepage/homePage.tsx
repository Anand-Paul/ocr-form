...
> {
    ...
    private homeProjectViewRef: React.RefObject<HomeProjectView> =
        React.createRef();
                    ...
                    <ul>
                        <li className="shadow">
                            ...
                            >
                                ...
                                />
                                <div className="title">
                                    {strings.homePage.prebuiltPredict.title}
                                </div>
                                <div className="description">
                                    {
                                        strings.homePage.prebuiltPredict
                                            .description
                                    }
                                </div>
                            </a>
                        ...
                        <li className="shadow">
                            ...
                            >
                                ...
                                />
                                <div className="title">
                                    {strings.homePage.layoutPredict.title}
                                </div>
                                <div className="description">
                    ...
                    this.props.recentProjects.length > 0 && (
                        <div className="app-homepage-recent bg-lighter-1">
                            ...
                            >
                                ...
                                <span className="title">
                                    {strings.homePage.openCloudProject.title}
                                </span>
                            </div>
                            <CondensedList
    ...
    private loadSelectedProject = async (
        project: IProject,
        sharedToken?: {}
    ) => {
        try {
            const loadedProject = await this.props.actions.loadProject(
                project,
                sharedToken
            );
            ...
            if (
                ...
                error.errorCode === ErrorCode.SecurityTokenNotFound
            ) {
                toast.error(strings.errors.securityTokenNotFound.message, {
                    autoClose: 5000,
                });
            }
            if (
                error instanceof AppError &&
                error.errorCode === ErrorCode.ProjectInvalidSecurityToken
            ) {
                toast.error(
                    strings.errors.projectInvalidSecurityToken.message,
                    { autoClose: 5000 }
                );
            }
        }
    ...
    private freshLoadSelectedProject = async (project: IProject) => {
        ...
        const projectToken = this.props.appSettings.securityTokens.find(
        ...
        const storageProvider = StorageProviderFactory.createFromConnection(
            decryptedProject.sourceConnection
        );
        try {
            ...
            try {
                ...
                if (
                    ...
                    err.errorCode === ErrorCode.BlobContainerIONotFound
                ) {
            ...
            if (
                ...
                const reason = interpolate(
                    strings.errors.projectNotFound.message,
                    {
                        file: `${project.name}${constants.projectFileExtension}`,
                        container: project.sourceConnection.name,
                    }
                );
