...
> {
    public render() {
        ...
        return (
            <div>
                ...
                {this.props.children}
                <div
                    className="prediction-field-header"
                    style={{ marginTop: 28 }}
                >
                    <h6 className="prediction-field-header-field">
                        {" "}
                        Page # / Field name / Value
                    </h6>
                    <h6 className="prediction-field-header-confidence">
                        {" "}
                        Confidence
                    </h6>
                </div>
    ...
    private renderItem = (item: any, key: any) => {
        ...
        if (item?.type === "array" || item?.type === "object") {
            ...
            return item?.valueArray || item?.valueObject ? (
                <div
                    key={key}
                    onClick={() => {
                        this.onTablePredictionClick(
                            item,
                            this.getTagColor(item.fieldName)
                        );
                ...
                <>
                    ...
                    <li className="predictiontag-item-label-null mt-0 mb-1">
                        NULL
                    </li>
                </>
            );
                ...
                <div className="predictiontag-name-body">
                    {
                        <span
                            ...
                            className="predictiontag-name-text px-2"
                        >
                            {item.fieldName}
                        </span>
                    }
    ...
    private getPostProcessedValue = (prediction: any) => {
        ...
        if (type) {
            ...
            if (
                typeof postProcessedValue === "string" &&
                text !== postProcessedValue
            ) {
    ...
    private getPageNumberFrom = (item: any) => {
        ...
        if (item && item.type === "object" && item.valueObject) {
            for (const property of Object.keys(item.valueObject)) {
                const pageNumber = this.getPageNumberFrom(
                    item.valueObject[property]
                );
                if (pageNumber) {
                    return pageNumber;
                }
